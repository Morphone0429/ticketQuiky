...
let main = () => {
  ...
  function testBtnInfo(text) {
    ...
    let cur = className("android.widget.TextView").text(text).findOne();
  ...
  function getImagePoint({ tmpPath }) {
    ...
    let temp = images.read(tmpPath);
    if (!temp) {
      return null;
    }
    let jpgPathArr = tmpPath.split("/");
    let consolePath = jpgPathArr[jpgPathArr.length - 1];
    //截图并找图
    let p = null;
    try {
  ...
  function returnPoint({ tmpPath, callBack, timeOut }) {
    wait(
      () => {
        let point = getImagePoint({ tmpPath });
        return !!point && point;
      },
      {
        then(point) {
