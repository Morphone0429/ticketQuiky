...
var defaultPlayEtcStr = "05-17";
//默认最高票价，例如：800
var defaultMaxTicketPrice = "700";
//默认观演人，例如：观演人a,观演人b
var defaultViewers = "楼超强,林彤彤";

main();

function main() {
  var playEtcStr = defaultPlayEtcStr ? defaultPlayEtcStr : getPlayEtc(); //场次信息
  let playEtcArr = playEtcStr.split(",");
  console.log("监控购票场次：" + playEtcArr);
  var maxTicketPrice = defaultMaxTicketPrice
    ? defaultMaxTicketPrice
    : getMaxTicketPrice(); //最高票价
  console.log("监控最高票价：" + maxTicketPrice);
  var viewers = defaultViewers ? defaultViewers : getViewers();
  console.log("观演人：" + viewers); //观演人
...
function doSubmit(amount, viewers) {
  ...
  if (!isDebug) {
    ...
    for (let cnt = 0; payNowBtn.exists(); cnt++) {
      ...
      //直接猛点就完事了
      payNowBtn.findOne().click();
      sleep(50);
      if (cnt % 20 == 0) {
        log(
          "点支付次数:" + cnt + " 可继续等待或返回上一个界面继续刷新其他票档"
        );
      }
...
function descOrTextNotContains(node, str) {
  if (
    (node.text() != null && node.text().includes(str)) ||
